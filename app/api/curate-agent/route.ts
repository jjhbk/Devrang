...
/**
 * devrang AI Curator — RAG + Agentic Reasoning + Streaming
 */
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.warn("⚠️ Missing Gemini API key — devrang in offline mode");
      return NextResponse.json({
        author: "ai",
        text: "👋 devrang is currently offline. Please add your Gemini API key to `.env.local`.",
      });
    }
     ...
     */
    const vectorSearch = {
      description: "Search the devrang product catalog by semantic meaning.",
      ...
      execute: async ({ query }: { query: string }) => {
        console.log("🧭 devrang performing vector search for:", query);
    ...
    /**
     * 🎨 devrang's persona & behavior
     */
    const system = `
You are devrang — a warm, elegant AI shopping curator.
You use empathy, sensory storytelling, and taste-driven reasoning to help users find the perfect gifts or perfumes.
You have access to a "vectorSearch" tool that retrieves real products from devrang's catalog.
    ...
    return NextResponse.json(
      {
        error:
          error?.message ||
          "Something went wrong while generating devrang's response.",
      },
      { status: 500 }
    );
  }
